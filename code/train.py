...
parser.add_argument('--margin_dir', 
    default='../record_margin',
    help = 'margin_dir'
)
...
parser.add_argument('--path_save_log', default = './log',
    help = 'path_save_log'
)
...
parser.add_argument('--scale_margin', default = 1, type = float,
    help = 'scale_margin'
)
...
os.makedirs(args.margin_dir,exist_ok=True)
if args.gpu_id != -1:
    torch.cuda.set_device(args.gpu_id)
# Directory for Log
LOG_DIR = args.LOG_DIR + '/logs_{}/{}_{}_embedding{}_alpha{}_mrg{}_{}_lr{}_batch{}{}_scale{}'.format(args.dataset, args.model, args.loss, args.sz_embedding, args.alpha, 
                                                                                            args.mrg, args.optimizer, args.lr, args.sz_batch, args.remark,args.scale_margin)
# Wandb Initialization
# wandb.init(project=args.dataset + '_ProxyAnchor', notes=LOG_DIR)
# wandb.config.update(args)mrg
os.makedirs(args.path_save_log,exist_ok=True)
visualization = tf.summary.create_file_writer(args.path_save_log)

os.chdir('../data/')
data_root = os.getcwd()
print("seeed ", args.seed)
random.seed(int(args.seed))
np.random.seed(int(args.seed))
torch.manual_seed(int(args.seed))
torch.cuda.manual_seed_all(int(args.seed)) # set random seed for all gpus
...
if args.model.find('googlenet')+1:
    ...
    model = Resnet50(embedding_size=args.sz_embedding, pretrained=False, is_norm=args.l2_norm, bn_freeze = args.bn_freeze)
...
elif args.loss == 'AdaptiveProxyAnchorLoss':
...
elif args.loss == 'MultiProxyAnchorLoss':
    criterion = losses.AdaptiveProxyAnchorLoss(nb_classes=nb_classes, sz_embed=args.sz_embedding, mrg=args.mrg, alpha=args.alpha,\
                                               nb_proxies=args.nb_proxies, scale_margin=args.scale_margin).cuda()
...
if args.loss in ['Proxy_Anchor', 'AdaptiveProxyAnchorLoss','Proxy_Anchor_adaptive_1_margin']:
    if args.loss != 'Proxy_Anchor':
        param_groups.append({'params': criterion.mrg,'lr':float(args.lr) })
...
if args.optimizer == 'sgd': 
    ...
    print(type(args.lr), type(args.weight_decay))
...
best_epoch = 0
import pandas as pd
if args.loss == 'Proxy_Anchor_adaptive_1_margin':
    df_margin = pd.DataFrame(columns = ['margin' for i in range(1)])
#     print(df_margin.columns)
elif args.loss == 'AdaptiveProxyAnchorLoss':
    df_margin = pd.DataFrame(columns = ['margin_'+str(i) for i in range(criterion.nb_classes)])
    print(df_margin.columns.shape)
record_epochs = []
recall_1 = []
import time
total_time = 0.0
for epoch in range(0, args.nb_epochs):
    ...
    for batch_idx, (x, y) in pbar:                         
        ...
        if args.loss == 'Proxy_Anchor' or args.loss == 'AdaptiveProxyAnchorLoss' or args.loss =='Proxy_Anchor_adaptive_1_margin':
        ...
        pbar.set_description(
            'Train Epoch: {} [{}/{} ({:.0f}%)] Loss: {:.6f}'.format(
                ...
                loss.item()))
    total_time = total_time + float(time.time()-time1)
    losses_list.append(np.mean(losses_per_epoch))
#     wandb.log({'loss': losses_list[-1]}, step=epoch)
    tf.summary.scalar('./loss/{}'.format('train'), np.mean(losses_per_epoch), step=epoch)
    scheduler.step()
    
    if args.loss == 'AdaptiveProxyAnchorLoss' or args.loss == 'Proxy_Anchor_adaptive_1_margin':
        df_margin.loc[epoch] = param_groups[2]['params'][0].detach().cpu().numpy().reshape(-1,)
        df_margin.to_csv(args.margin_dir+"/record_margin.csv")
...
#                 wandb.log({"R@{}".format(K): Recalls[i]}, step=epoch)
                tf.summary.scalar('./loss/R@{}'.format(K), Recalls[i], step=epoch)
        elif args.dataset != 'SOP':
            for i in range(6):
#                 wandb.log({"R@{}".format(2**i): Recalls[i]}, step=epoch)
                tf.summary.scalar('./loss/R@{}'.format(2**i), Recalls[i], step=epoch)
        else:
            for i in range(4):
#                 wandb.log({"R@{}".format(10**i): Recalls[i]}, step=epoch)
#                 tf.summary.scalar('./loss/R@{}'.format(K), Recalls[i], step=epoch)
                tf.summary.scalar('./loss/R@{}'.format(10**i), Recalls[i], step=epoch)
        # Best model save
        record_epochs.append(epoch)
        recall_1.append(Recalls[0])
        record = pd.DataFrame({
            'epochs': record_epochs,
            'recall_1':recall_1
        })
        record.to_csv(args.margin_dir+"/record.csv",index=False)
...
print("training time : ", total_time/args.nb_epochs)